...
(let ((awaken-blue-light "#89BDFF")
  ...
  (defun add-custom-type-mod-keyw()
    (font-lock-add-keywords nil
                            '(
                              ...
                              ("\\<\\(noexcept\\)" .  'font-lock-type-mods-face)
                              ("\\<\\(nullptr\\)" .  'font-lock-type-mods-face)
                              ("\\<\\(static_assert\\)" .  'font-lock-type-mods-face)
                              ("\\<\\(final\\)" .  'font-lock-type-mods-face)
                              ("\\<\\(alignof\\)" .  'font-lock-type-mods-face)
                              ("\\<\\(alignas\\)" .  'font-lock-type-mods-face)
                              ("\\<\\(decltype\\)" .  'font-lock-type-mods-face)
                              )
                            )
    )
  ;; (defun add-custom-type-mod-keyw()
  ...
  (custom-theme-set-faces
   ...
   `(font-lock-my-zfuncs-face     ((t (:foreground ,function-names))))
   ;; CUA
...
;; (defun add-my-functions-name-keyw()
;;     (font-lock-add-keywords nil
;;                             '(("\\s\"?\\(\\(\\sw\\|\\s_\\)+\\(<-\\)?\\)\\s\"?*\\s-*(" . font-lock-my-functions-face)))
;;                             )

;; (add-hook 'c++-mode-hook 'add-my-functions-name-keyw)

;;`(font-lock-my-functions-face   ((t (:foreground ,function-names))))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  ;;   (font-lock-add-keywords nil
  ;;                           '(
  ;;                             ("\\(\\w+\\)" . 'font-lock-function-calls-face)
  ;;                             )
  ;;                           )
  ;;   )
